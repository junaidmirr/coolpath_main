...
export default function App() {
  ...
  const globalAudioWebviewRef = useRef<WebView>(null);
  ...
  const speakGlobalText = async (text: string) => {
    ...
    // 4. Quaternary fallback: WebView audio player
    globalAudioWebviewRef.current?.injectJavaScript(
      `window._speakText && window._speakText(${JSON.stringify(cleanSpoken)}); true;`
    );
  };
  ...
  const startNavigation = async (mode: 'real' | 'simulated') => {
    ...
    globalAudioWebviewRef.current?.injectJavaScript('window._unlockAudio && window._unlockAudio(); true;');
    ...
    <SafeAreaView style={[styles.root, { backgroundColor: theme.bg }]}>
        ...
        <View style={StyleSheet.absoluteFill}>
          ...
          {uiVisible && !isNavigating && (
            <>
              ...
              {statusToast && (
                <View style={[styles.statusToastCard, { backgroundColor: theme.topCardBg, borderColor: theme.border }]}>
          ...
          {uiVisible && response && !isNavigating && (
            <Animated.View style={[styles.sheet, { height: sheetHeightAnim, backgroundColor: theme.sheetBg }]}>
      ...
      {activeTab === 'history' && (
        <View style={styles.tabContainer}>
          ...
          <ScrollView contentContainerStyle={styles.historyListContainer} showsVerticalScrollIndicator={false}>
            {historyList.length === 0 ? (
      ...
      {!isNavigating && (
        <View style={[styles.bottomNav, { backgroundColor: theme.navBg, borderColor: theme.border }]}>
          {/* Map Tab */}
      ...
      {/* 🔊 PERSISTENT GLOBAL AUDIO TTS WEBVIEW PLAYER */}
      <View style={{ width: 1, height: 1, opacity: 0.01, position: 'absolute', bottom: 0, left: 0, pointerEvents: 'none' }}>
        <WebView
          ref={globalAudioWebviewRef}
          originWhitelist={['*']}
          source={{ html: GLOBAL_AUDIO_ENGINE_HTML, baseUrl: 'https://localhost' }}
          javaScriptEnabled
          domStorageEnabled
